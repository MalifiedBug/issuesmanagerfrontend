    ...
    <div>    
      ...
      <div>
        <div className="grid grid-cols-3 text-xl bg-slate-400">
          <h1 className="self-center font-serif font-bold text-left lg:pl-8">Assign Permissions:</h1>
        ...
        <div className="flex flex-row justify-between">
          <h1 className="text-2xl font-serif font-bold text-left p-4">Admin - {adminName}</h1>
          <select onChange={(e)=>{
          ...
          <select onChange={(e)=>{
            if(e.target.value === "pending"){
              let newDat = data.filter((dat)=>dat.status === 'pending');
              setNewData(newDat);
            }else if(e.target.value === "resolved"){
              let newDat = data.filter((dat)=>dat.status === 'resolved');
              setNewData(newDat)
              console.log(newDat)
          ...
          <h1 className="self-center text-md font-semibold">Delete Resolved Queries <IconButton onClick={()=>{deleteQueries()}} color="error"><DeleteIcon /></IconButton></h1>
        </div>
